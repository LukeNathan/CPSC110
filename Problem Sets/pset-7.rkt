...
(check-expect (decoder (list "c" "o" "w") empty) "cow")
(check-expect (decoder (list "h") (list "keep")) "h")
(check-expect (decoder (list "i") (list "space")) " ")
(check-expect (decoder (list "j") (list "remove")) "")
(check-expect (decoder (list "h" "e" "l" "l" "o" "!")
                       (list "keep" "keep" "remove" "space" "remove" "keep"))
              "he !")
(check-expect (decoder (list "g" "o" "o" "d" "b" "y" "e")
                       (list "remove" "keep" "space"))
              "o dbye")
(check-expect (decoder (list "c" "o" "w")
                       (list "keep" "space" "keep" "remove" "space" "keep"))
              "c w")

;; CROSS PRODUCT OF TYPE COMMENTS TABLE  
;;
;;           ListOf1String empty    (cons 1String ListOf1String) 
;; Operations       
;;                             
;; empty                   "" [1]   (string-append (first l)      [2]
;;                                                 (decoder (rest l) ops)) 
;;
;; "keep"                  "" [1]   (string-append (first l)      [3]
;;                                            (decoder (rest l) (rest ops)))
;;
;; "space"                 "" [1]   (string-append " "            [4]
;;                                            (decoder (rest l) (rest ops)))
;;
;; "remove"                "" [1]   (decoder (rest l) (rest ops)) [5]

(@template-origin 2-one-of)

(define (decoder l ops) 
            (cond [(empty? l) ""]                                         ;(1)
                  [(empty? ops) (string-append (first l) 
                                               (decoder (rest l) empty))] ;(2)
                  [(string=? "keep" (first ops))
                   (string-append (first l)
                                  (decoder (rest l) (rest ops)))]         ;(3)
                  [(string=? "space" (first ops))
                   (string-append " " (decoder (rest l) (rest ops)))]     ;(4)
                  [else
                   (decoder (rest l) (rest ops))]))                       ;(5) 
...
(check-expect (three-criteria-courses C311) (list 311))
(check-expect (three-criteria-courses C312) empty)
(check-expect (three-criteria-courses C221) (list 313 317))
