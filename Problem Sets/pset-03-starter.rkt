...
;;
;; NOTE, for simplicity the spider lecture solution has a small bug in it.
;; Here's the bug.  Suppose a window is 400 high, how many numbers are in
;; the interval [0, 400]? Is it 400? If 0 is the number of the first interval,
;; what's the number of the last one?
;;
;; These constants resolve that bug.
;;

(define TOP (+ 0        BALL-RADIUS))  ;these constants define the "inner box"
(define BOT (- HEIGHT 1 BALL-RADIUS 1));that constrains the center of the ball
(define LEF (+ 0        BALL-RADIUS))  ;
(define RIG (- WIDTH  1 BALL-RADIUS 1));
...
(define B2 (make-ball (/ WIDTH 3) (/ HEIGHT 2) -4 3))
(define B3 (make-ball (/ WIDTH 2) (/ HEIGHT 3) 5 10))
...
(check-expect (render-ball B1)
              (place-image BALL (/ WIDTH 2) (/ HEIGHT 2) MTS))

(check-expect (render-ball B2)
              (place-image BALL (/ WIDTH 3) (/ HEIGHT 2) MTS))

(check-expect (render-ball B3)
              (place-image BALL (/ WIDTH 2) (/ HEIGHT 3) MTS))
...
#|        (place-image BALL ball-x b MTS) 
       (place-image BALL ball-y b MTS) |#
...
(check-expect (touch-left? (make-ball (+ LEF 5) BOT -4 3)) false)
(check-expect (touch-left? (make-ball (+ LEF 4) BOT -4 3))  true)
(check-expect (touch-left? (make-ball (+ LEF 1) BOT -2 3))  true)
(check-expect (touch-left? (make-ball (+ LEF 0) BOT  2 3)) false)
...
(check-expect (touch-right? (make-ball (- RIG 5) BOT  4 3)) false)
(check-expect (touch-right? (make-ball (- RIG 4) BOT  4 3))  true)
(check-expect (touch-right? (make-ball (- RIG 1) BOT  2 3))  true)
(check-expect (touch-right? (make-ball (- RIG 0) BOT -2 3)) false)
...
(check-expect (bounce-left (make-ball (+ LEF 3) (- BOT 1) -2 4))
              (make-ball (+ LEF 1) (- BOT 1) 2  4))
(check-expect (bounce-left (make-ball (+ LEF 6) (- BOT 2) -3 7))
...
(check-expect (bounce-right (make-ball (+ RIG 3) (- BOT 2) 2 4))
              (make-ball (- RIG 1) (- BOT 2) -2  4))
(check-expect (bounce-right (make-ball (+ RIG 6) (- BOT 1) 3 7))
              (make-ball (- RIG 1) (- BOT 1) -3 7))
...
(check-expect (glide (make-ball 150 200 5 10))
              (make-ball 155 210 5 10))

(check-expect (glide (make-ball 250 100 -5 -10))
              (make-ball 245 90 -5 -10))
