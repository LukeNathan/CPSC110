...
#;(check-expect (solve (list (make-slot "A" 1)
                           (make-slot "B" 1)
                           (make-slot "C" 2))
                     (list (make-ta "Joe" (list "B"))
                           (make-ta "Steve" (list "A" "B"))
                           (make-ta "Jim" (list "C"))))
              (list (list "Joe" "B") (list "Steve" "A") (list "Jim" "C")))
#;(check-expect (solve (list (make-slot "A" 1)
                           (make-slot "B" 1)
                           (make-slot "C" 2))
                     (list (make-ta "Joe" (list "B"))
                           (make-ta "Steve" (list "A" "B"))
                           (make-ta "Jim" (list "C"))
                           (make-ta "Jemothy" (list "A" "B" "C"))))
              (list (list "Joe" "B") (list "Steve" "A")
                    (list "Jim" "C") (list "Jemothy" "C")))
#;(check-expect (solve (list empty) (list (make-ta "Po" (list "P")))) false) 
#;(check-expect (solve (list empty) (list empty)) false) 
#;(check-expect (solve (list (make-slot "A" 1)) (list empty)) false)
(check-expect (solve (list (make-slot "Z" 1)) (list (make-ta "e" (list "Z"))))
              (list (list "e" "Z")))
#;(check-expect (solve (list (make-slot "H" 1))
                     (list (make-ta "Herb" "H") (make-ta "Hov" "H")))
              (list (list "Herb" "H")))
#;(check-expect (solve (list (make-slot "H" 3))
                     (list (make-ta "Herb" "H") (make-ta "Hov" "H")))
              false)
              
(@template-origin arb-tree genrec try-catch encapsulated)

(define (solve los lota)
  (local [(define (solve--assn assn)
            (cond [(solved? assn) assn]
                  [else
                   (solve--loa (next-assn assn))]))

          (define (solve--loa loa)
            (cond [(empty? loa) false]
                  [else
                   (local [(define try (solve--assn (first loa)))]
                     (if (not (false? try))
                         try
                         (solve--loa (rest loa))))])) 
          
          (define (solved? loa) 
            (= (length loa) (length los)))

          (define (next-assn assn)
            (local [(define (next-slot los assn)
                      (cond [(empty? assn) (first los)]
                            [else
                             (next-slot (rest los) (rest assn))]))

                   (define next-slot2 (next-slot los assn))

              (define (available? ta)
               (and (member (slot-lab next-slot2) (ta-avail ta)) 
                  (not (member (list (ta-name ta) (slot-lab next-slot2)) assn))
                  (< (length (filter (lambda (x) (member (ta-name ta) x)) assn))
                              3)))

                    (define (new-assns ta)
                      (cons (list (ta-name ta) (slot-lab next-slot2)) assn))]

              (map new-assns (filter available? lota))))]
