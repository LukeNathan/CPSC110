...
#reader(lib "htdp-intermediate-reader.ss" "lang")((modname handin-2022-11-03T22_47_01) (read-case-sensitive #t) (teachpacks ()) (htdp-settings #(#t constructor repeating-decimal #f #t none #f () #t)))
...
(define (all-a-contain? los w)
  (local [(define (stin? str)
            (string-contains? w str))
          (define (a? str) 
            (string=? "a" (substring str 0 1)))]
    (andmap stin? (filter a? los))))
...
(check-expect (brown-cat-names (list C1 C4)) (list "Whiskers" "Meow"))

(@template-origin fn-composition use-abstract-fn)
(define (brown-cat-names loc)
  (local [(define (brown? c)
            (string=? "brown" (cat-color c)))
          (define (id c)
            (string-append "" (cat-name c)))]
    (map id (filter brown? loc))))
...
(check-expect (count-short-names LOC1 10) 6)
  ...
  (local [(define (id c)
          ...
          (define (str-length str)
            (if (< (string-length str) n)
                true
                false))
          (define (count-c c loc)
            (+ 1 loc))]
    (foldr count-c 0 (filter str-length (map id loc)))))
...
(define (sum-age-c loc c)
  (local [(define (findc? cat)
            (if (string=? (cat-color cat) c)
                true
                false))
          (define (years ct)
            (cat-age ct))
          (define (count-c c loc)
            (+ c loc))]
    (foldr count-c 0 (map years (filter findc? loc)))))
...
(@htdf fold-treasure)
;;
(@signature (String Natural Natural X Z -> Y) (Y Z -> Z) X X X Z Treasure ->
            Y)
(check-expect (fold-treasure make-treasure cons "buried" "sunken" "locked"
                             empty T10) T10)
(check-expect (local [(define (c1 tl ta td fs fl)
                        (+ ta fl))]
                (fold-treasure c1 + "" "" "" 0 T10)) 287) 
...
(check-expect (more-than-n T1 32) (list "E"))
(check-expect (more-than-n T1 63) (list "E"))
(check-expect (more-than-n T1 64) empty)
(check-expect (more-than-n T3 0) (list "B" "E" "F"))
(check-expect (more-than-n T3 16) (list "B" "E" "F"))
(check-expect (more-than-n T10 0)
              (list "A" "B" "E" "F" "C" "D" "G" "H" "J" "I"))
(check-expect (more-than-n T10 17)
              (list "A" "B" "E" "F" "C" "D" "G" "H" "I"))
(check-expect (more-than-n T10 100)
            (list "G" "I"))
(check-expect (more-than-n T10 70)
              (list "C" "D" "G" "I"))
(check-expect (more-than-n T10 17)
              (list "A" "B" "E" "F" "C" "D" "G" "H" "I"))
(check-expect (more-than-n T10 18)
              (list "A" "E" "F" "C" "D" "G" "H" "I"))
(check-expect (more-than-n T10 19)
              (list "A" "E" "F" "C" "D" "G" "H" "I"))



(define (more-than-n t n)
  (local [(define (c1 tl ta td fs fl)
            (if (> (* fs ta) n)
                (cons tl fl) 
                fl)) 
          (define (c2 t lot)
            (append t lot))]
    (fold-treasure c1 c2 2 4 3 empty t)))  
