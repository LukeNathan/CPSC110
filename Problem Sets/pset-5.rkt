...
#reader(lib "htdp-beginner-reader.ss" "lang")((modname |pset-05-starter (1)|) (read-case-sensitive #t) (teachpacks ()) (htdp-settings #(#t constructor repeating-decimal #f #t none #f () #t)))
...
(define (main g)
  (big-bang g
    (on-draw   render-game)    ;ListOfBall -> Image
    (on-tick   next-game)      ;ListOfBall -> ListOfBall
...
(check-expect (render-game (make-game empty (/ WIDTH 2)))
              (place-image PADDLE (game-paddle G1) PADDLE-CTR-Y MTS))
(check-expect (render-game (make-game (cons (make-ball 10 20 3 4)
                   (cons (make-ball 30 40 1 2)
                         empty)) 3))
 (place-image PADDLE 3 PADDLE-CTR-Y
...
(@htdf next-game)
(@signature Game -> Game)
;; produce game with next balls and caught balls

(check-expect (next-game G1)
              (make-game (cons (make-ball 306.5 264.5 4 -3) empty) 302.5))
...
(@signature Game -> Game)
;; produce game that deletes caught balls

(check-expect (game-with-caught-balls G1)
              (make-game (cons (make-ball 302.5 267.5 4 -3) empty) 302.5))
(check-expect (game-with-caught-balls (make-game empty (/ WIDTH 2)))
              (make-game empty 302.5))
...
(define (game-with-caught-balls g)
  (make-game (catch-balls (game-balls g) (game-paddle g))
             (game-paddle g)))
...
(check-expect (catch-balls empty 10) empty)
(check-expect (catch-balls LOB1 5) LOB1)
(check-expect (catch-balls (cons (make-ball 50 PADDLE-CTR-Y 5 5) empty) 50)
              empty)

;(define (catch-balls lob) lob)
 ...
 (define (catch-balls lob p)
   (cond [(empty? lob) (... p)]
         [else
          (... p
               (fn-for-ball (first lob))
               (catch-balls (rest lob)))])))

(define (catch-balls lob p)
  (cond [(empty? lob) empty]
        [else
         (append (if (touch-paddle? (first lob) p)
                     empty
                     (list (first lob)))
...
(@signature Game -> Game)
;; produce a game with next balls
(check-expect (game-with-next-balls (make-game empty (/ WIDTH 2)))
...
(@template (define (game-with-next-balls g)
             (... (next-balls (game-balls g))
                  (game-paddle g))))
...
(check-random (handle-mouse (make-game empty 12) 100 200 "button-down")
              (make-game
               (cons (make-ball 100 200 (- 5 (random 11)) (- 5 (random 11)))
                    empty) 12))
(check-random (handle-mouse (make-game (cons (make-ball 10 20 -3 3) empty) 50)
                            300 100 "button-down")
              (make-game
               (cons (make-ball 300 100 (- 5 (random 11)) (- 5 (random 11)))
                    (cons (make-ball 10 20 -3 3) empty)) 50))
(check-random (handle-mouse (make-game empty (/ WIDTH 2)) 100 200 "button-up")
              (make-game empty (/ WIDTH 2)))
(check-random (handle-mouse (make-game (cons (make-ball 10 20 -3 3) empty) 111)
                            100 200 "button-up")
              (make-game (cons (make-ball 10 20 -3 3) empty) 111))
#;
...
;; clear all balls if space key pressed;
;; move paddle right if right key pressed by PADDLE-MOVE-PER-KEY
;; move paddle left if left key pressed by PADDLE-MOVE-PER-KEY
;;  else do nothing
(check-expect (handle-key (make-game
                           (cons (make-ball (/ WIDTH 2) (/ HEIGHT 2) 2 4)
...
(check-expect (handle-key G1 "left")
              (make-game (cons B1 empty) (- (/ WIDTH 2) PADDLE-MOVE-PER-KEY)))
