...
(@htdf layer)
(@signature Natural Image -> Image)
;; produce a row of img with length n
(check-expect (layer 0 COOKIES) empty-image)
(check-expect (layer 1 SQUARE) SQUARE)
(check-expect (layer 2 COOKIES) (beside COOKIES COOKIES))
(check-expect (layer 3 SQUARE) (beside SQUARE SQUARE SQUARE))

;(define (layer n img) img)
...
(check-expect (sink (cons "solid"
                          (cons "solid"
                                (cons "bubble" (cons "bubble" empty)))))
              (cons "bubble" (cons "solid" (cons "solid"
                                                 (cons "bubble" empty)))))
...
;; swap solid, keep list if bubble
