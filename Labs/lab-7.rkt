...
;; produce true if an answer is found in the tree after n questions
...
#;(define (n-count-y ynt)
  (cond [(string? ynt) 0]
        [else
         (if (string? (yntree-y ynt))
             (add1 0)
             (n-count-y (yntree-y ynt)))]))

#;(define (n-count-n ynt)
  (cond [(string? ynt) 0]
        [else
         (if (not (string? (yntree-n ynt)))
             (+ 1 (n-count-n (yntree-n ynt)))
             (n-count-n (yntree-n ynt)))]))
...
;;lop2                      0       (cons Player ListOfPlayer)
;;lop1
;;
;;empty                      empty   empty
;;
;;(cons Player ListOfPlayer) empty  (cons (make-match (first lop1) (first lop2))
;;                                  (set-matches (rest lop1) (rest lop2)))


(check-expect (set-matches LOPL0 LOPL1) empty)
(check-expect (set-matches LOPL1 LOPL0) empty)
...
(define (set-matches lop1 lop2)
  (cond [(or (empty? lop1) (empty? lop2)) empty]
        [else (cons (make-match (first lop1) (first lop2))
                    (set-matches (rest lop1) (rest lop2)))]))
